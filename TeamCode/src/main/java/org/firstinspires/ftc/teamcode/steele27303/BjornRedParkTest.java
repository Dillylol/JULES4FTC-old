...
public class BjornRedParkTest extends OpMode {
    ...
    public void init() {
        ...
        follower.setStartingPose(new Pose(111.000, 135.000, Math.toRadians(0))); // RED facing left (180) - Mirrored
        // User path: setConstantHeadingInterpolation(Math.toRadians(180))
        // So start heading should likely be 180.
        ...
        telemetry.addLine("Bjorn Red Park Test Initialized");
        telemetry.update();
    }
    ...
    private void updateTurretLogic() {
        // Use built-in position tracking for better consistency with TeleOp
        turretControl.setTargetPosition(
            BjornConstants.FieldPositions.RED_SCORING_X,
            BjornConstants.FieldPositions.RED_SCORING_Y
        );
        turretControl.setPositionTrackingEnabled(true);
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            ...
            ).setConstantHeadingInterpolation(Math.toRadians(0)) // Fixed: Mirrored to 18
            .build();
        }
    }
}
